// shooter_stages_colors_fatter_fighters_less_flat.cpp
// Infinite grassy ground (no road), off-road buildings with varied size/color and colliders,
// enemies fly higher and are clamped above buildings, sky billboard stroke text (non-mirrored),
// centered bold/big intro overlay (authors in yellow→light-red), and trees.
...
// Utilities
float frand(float a, float b) { return a + float(rand()) / RAND_MAX * (b - a); } // rng [web:108]
...
bool aabbCollision(float ax, float ay, float az, float aw, float ah, float ad,
    ...
    return fabs(ax - bx) * 2.0f < (aw + bw) &&
        ...
        fabs(az - bz) * 2.0f < (ad + bd); // AABB overlap [web:43]
}
...
void drawStars3D() {
    glDisable(GL_LIGHTING); glPointSize(2.0f); glBegin(GL_POINTS); glColor3f(1, 1, 1);
    for (const auto& s : stars) glVertex3f(s.x, s.y, s.z);
    glEnd(); glEnable(GL_LIGHTING);
}

// Fighter model (fatter, deeper) [web:108]
void drawFighterFlatColored(float x, float y, float z, float scale, float r, float g, float b) {
    glPushMatrix();
    glTranslatef(x, y, z);
...
static void drawQuadTop(float x, float y, float z, float w, float d, float r, float g, float b) {
    ...
    glColor3f(r, g, b); // sets current color for subsequent vertices [web:105][web:108]
    glBegin(GL_QUADS);
...
static void drawTree(float x, float z, float h = 10.0f) {
    GLUquadric* q = gluNewQuadric(); // GLU cylinder helper
    float trunkH = h * 0.4f;
    float crownH = h * 0.7f;
    glPushMatrix();
...
static void drawStrokeTextFacingCamera(const char* txt, float wx, float wy, float wz, float scale, const float camX, const float camZ) {
    float dx = camX - wx;
    float dz = camZ - wz;
    float yawDeg = atan2f(dx, dz) * 180.0f / 3.1415926f; // face camera
    glPushMatrix();
    glDisable(GL_LIGHTING);
...
static void drawEnvironmentSegment(float z0) {
    ...
    for (int k = 0; k < 6; ++k) {
        ...
        // sizes: mix of small and large
        float w = 14.0f + (k % 3) * 6.0f;
        float h = 24.0f + (k % 4) * 8.0f; // taller variety
        float d = 14.0f + ((k + 1) % 3) * 6.0f;
        // choose a base and vary it for distinct colors per building
        RGB base = (k % 3 == 0 ? RGB{ 0.28f,0.32f,0.70f }   // bluish
            : (k % 3 == 1 ? RGB{ 0.70f,0.30f,0.28f }  // warm red
        : RGB{ 0.30f,0.65f,0.35f }));// greenish
        float t = 0.15f * (k + 1);
        RGB vc = varyColor(base, t); // different color each building [web:88][web:108]
...
// ---------- Intro overlay (bold/big stroke font; authors in yellow→light red) ----------
...
void drawIntroOverlayStroke() {
    ...
    // Names (yellow to light red spectrum)
    drawCenteredStrokeLine("Al-Amin", cx, cy + 80, s2, 1.00f, 0.95f, 0.20f); // yellow [web:29]
    ...
    drawCenteredStrokeLine("Mahadi", cx, cy - 40, s2, 0.98f, 0.40f, 0.30f); // light red/orange

    // Dept (golden)
    drawCenteredStrokeLine("Dept of CSE,BUBT", cx, cy - 90, s3, 0.95f, 0.90f, 0.20f);

    // Prompt
    drawCenteredStrokeLine("Press any control (W/S/Arrows/Space/T) to start", cx, cy - 140, 0.24f, 0.9f, 0.95f, 1.0f);
}
...
void display() {
    ...
    // Ground (rebuild colliders)
    drawGroundEnvironment();

    // Sky text (billboard to camera, not mirrored)
    drawSkyText();
    ...
    drawText2D(10, HEIGHT - 55, "Arrows Turn/Ascend/Descend  W/S Forward/Back  Space Shoot  T Lock/Unlock  R Restart");
...
void update(int) {
    if (!gameOver) {
        ...
        for (auto& e : enemies) {
            ...
            // Keep enemies above buildings when overlapping in XZ
            for (const auto& bcol : buildingColliders) {
                ...
                if (overlapXZ) {
                    ...
                    if (e.y < minY) e.y = minY; // clamp above roof
                }
            }

            // Extra safety min altitude
...
void setupLights();
void initGL() { glEnable(GL_DEPTH_TEST); glShadeModel(GL_SMOOTH); setupLights(); glClearColor(0.02f, 0.03f, 0.07f, 1.0f); }
...
int main(int argc, char** argv) {
    ...
    glutCreateWindow("3D Shooter (Infinite Ground, Sky Text, Varied Buildings, Trees)");
    initGL(); initGame();
